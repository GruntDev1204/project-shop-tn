...
class AuthController extends Controller
{
    ...
    public function login2FA(Auth2FAREq $request)
    {
        ...
        $jwtToken = $user->remember_token;
        $user->remember_token = null;
        $user->save();
        $this->deleteOTP($user->hash_code, 'login');

        return $this->respondWithToken($this->validateRoleName($request->role, $request->email),  $jwtToken);
    }
    ...
    private function generateTOTP($secretKey, $labelApp, $userIdentity)
    {
    ...
    public function totpTest()
    {
        $secretKey = "KRUGS4ZANFZSAYJA";
        $labelApp = "login";
        $userIdentity = 'user@example.com';
        $qrEncode = $this->generateTOTP($secretKey, $labelApp, $userIdentity);

        return $this->returnJson([
            'qr_code' => "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=$qrEncode"
        ], 200, null);
    }

    private function isValidTOTP($secretKey, $otp)
    {
        return (new Google2FA())->verifyKey($secretKey, $otp);
    }

    public function verifiTOTP(Request $request)
    {
        $otp = $request->input('otp');
        $secretKey = "KRUGS4ZANFZSAYJA";
        if (!$otp) {
            return $this->returnJson(null, 422, " OTP are required.");
        }

        if ($this->isValidTOTP($secretKey, $otp)) {
            return $this->returnJson(null, 200, "OTP is valid.");
        } else {
            return $this->returnJson(null, 422, "Invalid OTP.");
        }
    }
}
